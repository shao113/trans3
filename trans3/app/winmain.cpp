...
void registerFonts(bool bRegister)
{
	...
	//SendMessageTimeout(HWND_BROADCAST, WM_FONTCHANGE, 0, 0,SMTO_BLOCK,2000,0);
}
...
void setUpGame()
{
	...
	// Run startup program.
	CProgram::verifyAndRun(g_mainFile.startupPrg);
...
STRING getMainFileName(const STRING cmdLine)
{
	...
	OPENFILENAME ofn = {
		...
		NULL, 0,
		(useAbsolutePath ? absolutePath : GAM_PATH),
		_T("Open Main File"),
...
int mainEntry(const HINSTANCE hInstance, const HINSTANCE /*hPrevInstance*/, const LPTSTR lpCmdLine, const int nCmdShow)
{
	...
	/*freopen("N:/tmp/Temp/log/log.txt", "w", stdout);
