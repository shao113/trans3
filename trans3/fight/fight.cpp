...
void enemyAttack(const int idx)
{
	...
	if (pEnemy->useCode)
	{
		...
		CProgram::verifyAndRun(pEnemy->prg);
		return;
	}
...
int statusEffectTick(LPSTATUS_PAIR pEffectPair, LPFIGHTER pOuterFighter)
{
	...
	if (pEffect->code)
	{
		...
		CProgram::verifyAndRun(pEffect->prg);
	}
...
void fightTest(const int moveSize)
{
	...
	if (!(((g_mainFile.fightType == 0) ? rand() : (g_pxStepsTaken / 32)) % g_mainFile.chances))
	{
		...
		{
			CProgram::verifyAndRun(g_mainFile.fightPrg);
		}
		else
		{
...
void rewardPlayers(const int gp, const int exp, const STRING prg)
{
	...
	// Run RPGCode program if set.
	CProgram::verifyAndRun(prg);
}
...
void runFight(const std::vector<STRING> enemies, const STRING background)
{
	...
	//` First ensure all the files exist.
	if (!CFile::fileExists(g_projectPath + BKG_PATH + background))
	{
		messageBox(_T("Could not find background: ") + background);
		return;
	}

	for (std::vector<STRING>::const_iterator it = enemies.begin(); it != enemies.end(); ++it)
	{
		if (!CFile::fileExists(g_projectPath + ENE_PATH + *it))
		{
			messageBox(_T("Could not find enemy: ") + *it);
			return;
		}
	}
	...
	else if (outcome == FIGHT_LOST)
	{
		...
		{
			CProgram::verifyAndRun(g_mainFile.gameOverPrg);
		}
	}
