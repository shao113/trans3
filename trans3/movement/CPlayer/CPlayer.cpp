...
void CPlayer::levelUp()
{
	...
	// Run the level up program, if set.
	CProgram::verifyAndRun(m_playerMem.charLevelUpRPGCode);
...
void CPlayer::addEquipment(const unsigned int slot, const STRING file)
{
	...
	CProgram::verifyAndRun(item.prgEquip);
	g_target = t;
}
...
void CPlayer::removeEquipment(const unsigned int slot)
{
	...
	g_target.p = this;
	g_target.type = ET_PLAYER;
	CProgram::verifyAndRun(item.prgRemove);
