...
TILE_TYPE CSprite::spriteCollisions(CSprite *&pSprite)
{
	...
	ZO_ITR i = g_sprites.find(this), pos;
...
bool CSprite::doBoardEdges(void)
{
	...
	{
		// This is a program.
		CProgram::verifyAndRun(fileName);
		return true;
	}
...
void CSprite::send(void)
{
	...
	CProgram::verifyAndRun(g_pBoard->enterPrg);
}
...
bool CSprite::programTest(const bool keypressOnly)
{
	...
	if (itm)
	{
		...
		// Run the activation program
		if (!itm->m_brdData.prgActivate.empty())
			CProgram::verifyAndRun(itm->m_brdData.prgActivate);
	...
	if (prg)
	{
		...
		// Run the program
		CProgram::verifyAndRun(prg->fileName);
