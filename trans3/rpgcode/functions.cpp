...
{
	STRING::iterator i = str.begin(), first, last;
	int depth = 0;
...
void mwin(CALL_DATA &params)
{
	...
	// Write the text.
	STRING str = spliceVariables(params.prg, params[0].getLit());
	g_mwin.cnvText->DrawText(0, g_mwin.nextLine, str, g_fontFace, g_fontSize, g_color, g_bold, g_italic, g_underline);
...
void run(CALL_DATA &params)
{
	...
	params.prg->end();
	CProgram::verifyAndRun(params[0].getLit());
}
